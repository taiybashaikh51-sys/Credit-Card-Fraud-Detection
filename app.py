...
if page == "🏠 Overview":
    ...
    with col1:
        ...
        fig3 = go.Figure()
        fig3.add_trace(go.Box(y=fraud_df['Amount'], name='Fraud', marker_color='#EF4444'))
        fig3.add_trace(go.Box(y=normal_df['Amount'], name='Normal', marker_color='#22C55E'))
    ...
    with col2:
        ...
        time_normal = df[df['Class']==0]['Time'].values
        fig4 = go.Figure()
        fig4.add_trace(go.Histogram(x=time_fraud, name='Fraud', marker_color='#EF4444', opacity=0.7))
        fig4.add_trace(go.Histogram(x=time_normal, name='Normal', marker_color='#22C55E', opacity=0.7))
    ...
    with col1:
        fig = go.Figure()
        fig.add_trace(go.Histogram(x=df[df['Class']==1][feature], name='Fraud', marker_color='#EF4444', opacity=0.7))
        fig.add_trace(go.Histogram(x=df[df['Class']==0][feature], name='Normal', marker_color='#22C55E', opacity=0.7))
    ...
    with col1: st.metric("✅ Accuracy", f"{acc*100:.1f}%")
    with col2: st.metric("🎯 Precision", f"{prec*100:.1f}%")
    with col3: st.metric("📊 Recall", f"{rec*100:.1f}%")
    with col4: st.metric("⚡ F1 Score", f"{f1*100:.1f}%")
    ...
    with col2:
        ...
        roc_auc = auc(fpr, tpr)
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=fpr, y=tpr, name=f'ROC (AUC = {roc_auc:.3f})', line=dict(color='#EF4444', width=2)))
        fig2.add_trace(go.Scatter(x=[0,1], y=[0,1], line=dict(dash='dash', color='#94A3B8'), name='Random'))
        fig2.update_layout(paper_bgcolor='#1A2333', plot_bgcolor='#1A2333',
            font=dict(color='#94A3B8'),
